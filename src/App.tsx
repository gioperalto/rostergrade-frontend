...
type Player = { id: string; name: string; position: string; team: string; potential: number; consistency: number; value: number; expert_consensus: number; injury_risk: number; roster_grade: number; position_rank: number };
  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      ...
      <section className="content"><div className="page-heading"><div><p className="eyebrow">THE BOARD</p><h1>{view === 'players' ? 'Player rankings' : view === 'offenses' ? 'Team offenses' : view === 'defenses' ? 'Defensive units' : 'Mock draft room'}</h1><p className="subhead">A broader view of fantasy value, built for decisions that last beyond draft day.</p></div><button className="primary"><Trophy size={17}/> My roster</button></div>
        {view === 'mock' ? <MockDraft players={players} /> : view === 'offenses' ? <Offenses teams={teams} players={players} /> : <><div className="insight"><div className="insight-icon"><Sparkles size={18}/></div><div><strong>RosterGrade is more than a projection.</strong><span>Potential measures ceiling and opportunity. Consistency measures how safely a player gets you there.</span></div><ChevronRight size={18}/></div><div className="toolbar"><div className="search"><Search size={17}/><input placeholder="Search player or team" value={search} onChange={e => setSearch(e.target.value)}/></div><div className="sort"><ArrowUpDown size={16}/><select value={sort} onChange={e => setSort(e.target.value)}><option value="roster_grade">RosterGrade</option><option value="potential">Potential</option><option value="consistency">Consistency</option><option value="value">Value</option><option value="name">Name</option></select></div><label className="scoring-input">PPR <input type="number" min="0" max="2" step="0.5" value={reception} onChange={e => setReception(Number(e.target.value))}/></label></div><div className="position-tabs">{positions.map(item => <button key={item} className={position === item ? 'tab selected' : 'tab'} onClick={() => setPosition(item)}>{item}</button>)}</div><div className="table-card"><div className="table-head"><span>Player</span><span>Position</span><span>RosterGrade</span><span>Potential</span><span>Consistency</span><span>Consensus</span><span>Injury risk</span><span>Value</span></div>{loading ? <div className="empty">Loading rankings…</div> : visiblePlayers.map((player, index) => <PlayerRow key={player.id} player={player} index={index}/>)}</div><div className="updated"><CalendarClock size={15}/> Rankings last refreshed {lastRefresh ? new Date(lastRefresh).toLocaleString() : 'not yet'} · source: {source || 'pending'} · refreshes at most once per day</div></>}
      </section>
    </main>
  </div>;
}
