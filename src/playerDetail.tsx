...
function DefenseEvents({ player }: { player: Player }) {
  ...
  return <section className="entity-section defensive-events"><SectionTitle title="Defensive event evidence"/><p className="muted">Auditable event-derived components only. Defensive grade/signal remains a separate strength measure.</p><div className="event-grid">{defensiveEventDefinitions().map(({ key, label }) => { const event = evidence.events[key]; return <div className="event-card" key={key}><span>{label}</span><strong>{event.season === null ? unavailable : event.season}</strong><small>Season · {event.perGame === null ? 'Per game unavailable' : `${event.perGame}/game`}</small><small>Scoring contribution · {event.scoringContribution === null ? 'Unavailable' : event.scoringContribution}</small><small className={event.source ? '' : 'unavailable-text'}>{event.source ? `Source: ${event.source}` : `Evidence ${event.status}`}</small></div>; })}</div><dl className="provenance"><div><dt>Evidence status</dt><dd>{evidence.status}</dd></div><div><dt>Evidence source</dt><dd>{evidence.source || unavailable}</dd></div><div><dt>Defensive grade / signal</dt><dd>{finite(player.defensive_grade) ? player.defensive_grade : unavailable} / {finite(player.defensive_signal) ? player.defensive_signal : unavailable}</dd></div></dl></section>;
}
