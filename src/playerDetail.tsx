...
function DefenseEvents({ player }: { player: Player }) {
  ...
  return <section className="entity-section defensive-events"><SectionTitle title="Defensive event evidence"/><p className="muted">Auditable event-derived components only. Defensive grade/signal remains a separate strength measure.</p><div className="event-grid">{defensiveEventDefinitions().map(({ key, label }) => { const event = evidence.events[key]; return <div className={`event-card event-${event.status}`} key={key}><div className="event-card-heading"><span>{label}</span><strong className={`event-status status-${event.status}`} aria-label={`${label} evidence status: ${event.status}`}>{event.status}</strong></div><strong className="event-season">{event.season === null ? unavailable : event.season}</strong><small>Season total · {event.season === null ? 'Unavailable' : 'reported'}</small><small>Per game · {event.perGame === null ? 'Unavailable' : event.perGame}</small><small>Scoring contribution · {event.scoringContribution === null ? 'Unavailable' : event.scoringContribution}</small><small className={event.source ? '' : 'unavailable-text'}>{event.source ? `Source: ${event.source}` : 'Source: Unavailable'}</small></div>; })}</div><dl className="provenance"><div><dt>Evidence status</dt><dd>{evidence.status}</dd></div><div><dt>Evidence source</dt><dd>{evidence.source || unavailable}</dd></div><div><dt>Defensive grade / signal</dt><dd>{finite(player.defensive_grade) ? player.defensive_grade : unavailable} / {finite(player.defensive_signal) ? player.defensive_signal : unavailable}</dd></div></dl></section>;
}
